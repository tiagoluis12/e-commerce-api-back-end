...
const createOrder = async (req, res) => {
  ...
  if (!cartItems || cartItems.length < 1) {
    throw new CustomError.BadRequestError("Please provide at least one item");
  }
  if (!tax || !shippingFee) {
  ...
  for (const items of cartItems) {
    const dbProduct = await Product.findOne({ _id: items.product });
    if (!dbProduct) {
      throw new CustomError.NotFoundError(
        `No product with id : ${items.product}`
      );
    }
